...
Reference only. Keep non-secret rebuild details here. Do not commit auth keys,
peer inventory, or `/var/lib/tailscale/` state.
...
SMB depends on [../samba/smb.conf.pi](../samba/smb.conf.pi), which allows only
localhost plus tailnet ranges.
