...
Run against `~/repo`:
```bash
REPO_DIR=~/repo python3 ~/src/dotfiles/scripts/sync_repo.py
```
...
Hardcoded "Repo Sync Error" breaks multi-repo use. Derive from `REPO_DIR.name`.
...
```bash
REPO_DIR=~/repo python3 ~/src/dotfiles/scripts/sync_repo.py
tail -10 ~/Library/Logs/repo-sync.log
```
